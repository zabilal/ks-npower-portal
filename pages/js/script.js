$(document).ready(function () {
    ...
    //FUNCTION TO VALIDATE EDUCATION DATA
...
// $('.hidden_for_sec').hide();
    ...
    $('#nysc_status_yes').change(function () {
        $('.nysc_cert').show();
        $('.nysc_completion').hide();
    })

    $('#nysc_status_no').change(function () {
        $('.nysc_cert').hide();
        $('.nysc_completion').show();
    })

    $('.next_btn').click(function () {
        let id_type = $('#id_type').val().trim();
        let id_number = $('#id_number').val().trim();
        let gov_id = $('#gov_id').val().trim();
        let passport = $('#passport').val().trim();

        if (id_type !== '' && id_number !== '' && gov_id !== '' && passport !== '') {
            next();
        } else {
            $('.emp_error').text('Please Fill in all required fields').css('color', '#dc3545');
            $('.id_type').addClass('error_border');
            $('.id_number').addClass('error_border');
            $('.gov_id').addClass('error_border');
            $('.passport').addClass('error_border');
        }
    })
